...
public class MicrosoftUserInfoSuccessResponse extends UserInfoSuccessResponse {
  ...
  public static MicrosoftUserInfoSuccessResponse parse(final HTTPResponse httpResponse) throws ParseException {
    ...
    if (httpResponse.getContentType().match(APPLICATION_JSON)) {
      try {
        return getMicrosoftUserInfoSuccessResponseFromJsonObject(httpResponse.getContentAsJSONObject());
      ...
      if (httpResponse.getContentType().match(APPLICATION_JWT)) {
        try {
          return getMicrosoftUserInfoSuccessResponseJWTObject(httpResponse.getContentAsJWT());
  ...
  private static MicrosoftUserInfoSuccessResponse getMicrosoftUserInfoSuccessResponseFromJsonObject(
      final JSONObject jsonObject) throws ParseException {
    jsonObject.put(SUB_CLAIM_NAME, getString(jsonObject, "id"));
    return new MicrosoftUserInfoSuccessResponse(new MicrosoftUserInfo(jsonObject));
  }

  private static MicrosoftUserInfoSuccessResponse getMicrosoftUserInfoSuccessResponseJWTObject(final JWT jwtObject)
      throws ParseException {
    return new MicrosoftUserInfoSuccessResponse(jwtObject);
  }
}
