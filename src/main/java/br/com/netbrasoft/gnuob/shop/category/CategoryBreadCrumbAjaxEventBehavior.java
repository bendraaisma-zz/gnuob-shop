...
public class CategoryBreadCrumbAjaxEventBehavior extends AjaxEventBehavior {
  ...
  public CategoryBreadCrumbAjaxEventBehavior(final String event, final BreadCrumbPanel caller, final IModel<SubCategory> model) {
    super(event);
    this.model = model;
  ...
  protected IBreadCrumbParticipant getParticipant(final String componentId) {
    final CategoryViewPanel categoryViewPanel =
        new CategoryViewPanel(componentId, this.breadCrumbModel, this.categoryModel, model, Model.ofList(model.getObject().getSubCategories()));
