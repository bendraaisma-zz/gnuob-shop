      ...
      class OrderRecordContainer extends WebMarkupContainer {
        ...
        protected void onInitialize() {
          ...
          add(new ProductCarousel("productCarousel", carouselImages).setOutputMarkupId(true).add(new PopoverBehavior(Model.of(getString("descriptionMessage")),
              Model.of(orderRecord.getProduct().getDescription()), new PopoverConfig().withHoverTrigger().withPlacement(Placement.left))));
          add(new Label("name"));
          add(new Label("product.stock.quantity"));
          add(new Label("amountWithDiscount",
              Model.of(NumberFormat.getCurrencyInstance().format(orderRecord.getProduct().getAmount().subtract(orderRecord.getProduct().getDiscount())))));
          ...
          add(new Loop("rating", FIVE_STARS_RATING) {
            ...
            protected void populateItem(LoopItem loopItem) {
              loopItem.add(new IconBehavior(loopItem.getIndex() < (orderRecord.getProduct().getRating() != null ? orderRecord.getProduct().getRating() : 0)
