...
public class WishListViewPanel extends Panel {
  ...
  /**
   * If no offers are available to show and select from a contract, this EmptyWishtListViewFragment
   * will be used to let the customer know that he has not a saved offer available.
   *
  ...
  class WishtListViewFragment extends Fragment {
    ...
    class OfferContainer extends WebMarkupContainer {
        ...
        protected void onInitialize() {
          ...
          add(new Loop("rating", FIVE_STARS_RATING) {
            ...
            protected void populateItem(LoopItem loopItem) {
              loopItem.add(new IconBehavior(loopItem.getIndex() < (offerRecord.getProduct().getRating() != null ? offerRecord.getProduct().getRating() : 0)
                  ? GlyphIconType.star : GlyphIconType.starempty));
            }
          });
          super.onInitialize();
        }
      ...
      class OfferRecordTotalContainer extends WebMarkupContainer {
        ...
        @AuthorizeAction(action = Action.RENDER, roles = {ShopRoles.ADMINISTRATOR})
        class RemoveAjaxButton extends BootstrapAjaxLink<String> {
          ...
          public RemoveAjaxButton() {
            super("remove", Model.of(WishListViewPanel.this.getString("removeFromWishListMessage")), Buttons.Type.Default,
                Model.of(WishListViewPanel.this.getString("removeFromWishListMessage")));
          }

          @Override
          public void onClick(AjaxRequestTarget target) {
            final Shopper shopper = shopperDataProvider.find((Shopper) WishListViewPanel.this.getDefaultModelObject());
        ...
        class SaveAjaxButton extends BootstrapAjaxLink<String> {
          ...
          public SaveAjaxButton() {
            super("save", Model.of(WishListViewPanel.this.getString("addToCartMessage")), Buttons.Type.Primary,
                Model.of(WishListViewPanel.this.getString("addToCartMessage")));
          }
          ...
          public void onClick(AjaxRequestTarget target) {
            ...
            shopper.setCart((Offer) OfferRecordTotalContainer.this.getDefaultModelObject());
